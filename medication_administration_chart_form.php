...
pheader("Medication Administration Chart Form");
$user_id;
$visit_id;
...
if(isset($_POST['submit']))
{
    ...
    $dates = $_POST['date'];
    $times = $_POST['time'];
    $mednames = $_POST['medname'];
    $doses = $_POST['dose'];
    $frequencies = $_POST['frequency'];
    $anupanams = $_POST['anupanam'];

    foreach ($dates as $key => $date) {
        $time = $times[$key];
        $medname = $mednames[$key];
        $dose = $doses[$key];
        $frequency = $frequencies[$key];
        $anupanam = $anupanams[$key];

        $sql = "INSERT INTO medication_adm_chart_form (user_id, visit_id, date, time, med_name, dose, frequency, anupanam, nurse_id, nurse_name, nurses_sign) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iisssssssss", $user_id, $visit_id, $date, $time, $medname, $dose, $frequency, $anupanam, $current_user_id, $current_user_name, $current_user_sign);
        $stmt->execute();
    }
 ...
 if (!((!in_array($current_user_role, $table_submit_roles) && !in_array($current_user_role,$sup_roles))||(($formeditable=="No") && (!in_array($current_user_role,$sup_roles)))))
    {
        ?>
<body id="body">
    <h1 class="text-center my-3">Medication Administration Chart Form</h1>
    <div class="container d-flex justify-content-center">
        <form method="POST" action="">
                ...
                <div class="patient">
                    ...
                    <div class="col">
                        <div class="form-group">
                            ...
                            <input type="text" name="medname[]" class="form-control" maxlength="50" value="">
                        </div>
                    </div>
                    <div class="col">
                        <div class="form-group">
                            <label for="dose">Dose</label>
                            <input type="text" name="dose[]" class="form-control" maxlength="20" value="">
                        </div>
                    </div>
                    <div class="col">
                        <div class="form-group">
                    ...
                    <div class="col">
                        <div class="form-group">
                            <label for="anupanam">Anupanam</label>
                            <input type="text" name="anupanam[]" class="form-control" maxlength="50" value="">
                        </div>
                    </div>
                </div>
                </div>
            ...
            <button type="button" class="btn btn-primary my-3 mx-2" onclick="addrow()">Add Dosage</button>
             <button type="button"class="btn btn-danger" onclick="removerow()">Remove Dosage</button>
    ...
    $sql = "SELECT COUNT(*) as count FROM $table_name WHERE visit_id=? AND (med_name LIKE CONCAT('%', ?, '%') OR dose LIKE CONCAT('%', ?, '%')  OR frequency LIKE CONCAT('%', ?, '%') OR anupanam LIKE CONCAT('%', ?, '%')) AND (nurse_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?)";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die("Error: " . $conn->error);
    }
    $stmt->bind_param("isssssssss", $visit_id, $searcho, $searcho, $searcho, $searcho, $nurn, $sdate, $ldate, $stime, $ltime);
    if (!$stmt->execute()) {
    ...
    $sql2 = "SELECT * FROM $table_name WHERE visit_id=? AND (med_name LIKE CONCAT('%', ?, '%') OR dose LIKE CONCAT('%', ?, '%')  OR frequency LIKE CONCAT('%', ?, '%') OR anupanam LIKE CONCAT('%', ?, '%')) AND (nurse_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?) ORDER BY date DESC, time DESC LIMIT $offset, $total_patvisrecords_per_page";
        $stmt2 = $conn->prepare($sql2);
        if (!$stmt2) {
            die("Error: " . $conn->error);
        }
        $stmt2->bind_param("isssssssss", $visit_id, $searcho, $searcho, $searcho, $searcho, $nurn, $sdate, $ldate, $stime, $ltime);
        if (!$stmt2->execute()) {
    ...
    <div class="container">
        <div class="row">
            <div class="col">
                <h4 class="text-center no-print">Medication Administration Chart  Records</h4>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <form class="my-3 no-print" action="" method="GET">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="Search by Medicine Name, Dose, Frequency, Anupanam" name="searcho" value="<?php echo $searcho; ?>">
                            </div>
                        </div>
                    </div>
