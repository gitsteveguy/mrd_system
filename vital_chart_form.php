...
if (isset($_POST['submit'])) {
    ...
    foreach ($dates as $key => $date) {
        ...
        $sql = "INSERT INTO vital_chart_form (visit_id, user_id, date, time, temp, pulse, bp, weight, remarks, doctor_id, doctor_name, doctors_sign) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iisssssssiss", $visit_id, $user_id, $date, $time, $temp, $pulse, $bp, $weight, $remark, $current_user_id, $current_user_name, $current_user_sign);
        $stmt->execute();
    }
...
<?php 
 if (!((!in_array($current_user_role, $table_submit_roles) && !in_array($current_user_role,$sup_roles))||(($formeditable=="No") && (!in_array($current_user_role,$sup_roles)))))
    {
        ?>
    <h1 class="text-center my-3">Vital Chart</h1>
                    ...
                    <div class="row my-2">
                        ...
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input type="text" name="temp[]" maxlength="10" class="form-control" value="">
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group"><br>
                                <label for="pulse">Pulse</label>
                                <input type="text" name="pulse[]" maxlength="10" class="form-control" value="">
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group"><br>
                                <label for="bp">B.P</label>
                                <input type="text" name="bp[]" maxlength="10" class="form-control" value="">
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group"><br>
                                <label for="weight">Weight</label>
                                <input type="text" name="weight[]" maxlength="10" class="form-control" value="">
                            </div>
                        </div>
                        <div class="col"><br>
                            <div class="form-group">
                                <label for="remarks">Remarks</label>
                                <input type="text" name="remarks[]" maxlength="200" class="form-control" value="">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <button type="button" class="btn btn-primary my-3 mx-2" onclick="addrow()">Add Vitals</button>
    <button type="button" class="btn btn-danger" onclick="removerow()">Remove Vitals</button>
    ...
    $sql = "SELECT COUNT(*) as count FROM $table_name WHERE visit_id=? AND (temp = ? OR pulse = ? OR bp = ? OR weight = ? OR remarks LIKE CONCAT('%', ?, '%')) AND (doctor_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?)";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
    ...
    $sql2 = "SELECT * FROM $table_name WHERE visit_id=? AND (temp = ? OR pulse = ? OR bp = ? OR weight = ? OR remarks LIKE CONCAT('%', ?, '%')) AND (doctor_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?) ORDER BY date DESC, time DESC LIMIT $offset, $total_patvisrecords_per_page";
    ...
    <div class="container">
        <div class="row">
            <div class="col">
                <h4 class="text-center no-print">Vital Chart Records</h4>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <form class="my-3 no-print" action="" method="GET">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="Search by Temperature, Pulse, B.P, Weight or Remarks" name="searcho" value="<?php echo $searcho; ?>">
                            </div>
                        </div>
                    </div>
