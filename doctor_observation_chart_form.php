...
if(isset($_POST['submit']))
{
    ...
    $medmods = $_POST['medmod'];
    $promods = $_POST['promod'];
    ...
    foreach ($dates as $key => $date) {
        ...
        $promod = $promods[$key];
        $observ1 = $observe[$key];
        $sql = "INSERT INTO doctor_observe_chart_form (visit_id, user_id, date, time, observe, medmod, promod, doctor_id, doctor_name, doctors_sign) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iisssssiss", $visit_id, $user_id, $date, $time, $observ1, $medmod, $promod, $current_user_id, $current_user_name, $current_user_sign);
        $stmt->execute();
    }
...
<?php 
 if (!((!in_array($current_user_role, $table_submit_roles) && !in_array($current_user_role,$sup_roles))||(($formeditable=="No") && (!in_array($current_user_role,$sup_roles)))))
    {
        ?>
    <h1 class="text-center my-3">Doctor's Observation Chart</h1>
        ...
        <form method="POST" action="">
            <div id="patient-fields">
                <div class="patient">
                    ...
                    <div class="col">
                        <div class="form-group">
                            <label for="observe">Doctor's Observation</label>
                            <textarea class="textarea" maxlength="200" rows="5" cols="30" name="observe[]" class="form-control"></textarea>
                        </div>
                    </div>
                    <div class="col">
                        <div class="form-group">
                            <label for="medmod">Medicine Modification</label>
                            <input type="text" name="medmod[]" maxlength="200" class="form-control" value="">
                        </div>
                    </div>
                    <div class="col">
                        <div class="form-group">
                            <label for="promod">Procedure Modification</label>
                            <input type="text" name="promod[]" maxlength="200" class="form-control" value="">
                        </div>
                    </div>
                </div>
                </div>
            ...
            <button type="button" class="btn btn-primary my-3 mx-2" onclick="addrow()">Add Observation</button>
             <button type="button"class="btn btn-danger" onclick="removerow()">Remove Observation</button>
    ...
    $sql = "SELECT COUNT(*) as count FROM $table_name WHERE visit_id=? AND (observe LIKE CONCAT('%', ?, '%') OR medmod LIKE CONCAT('%', ?, '%') OR promod LIKE CONCAT('%', ?, '%')) AND (doctor_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?)";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die("Error: " . $conn->error);
    }
    $stmt->bind_param("issssssss", $visit_id, $searcho, $searcho, $searcho, $docn, $sdate, $ldate, $stime, $ltime);
    if (!$stmt->execute()) {
    ...
    $sql2 = "SELECT * FROM $table_name WHERE visit_id=? AND (medmod LIKE CONCAT('%', ?, '%') OR promod LIKE CONCAT('%', ?, '%')) AND (doctor_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?) ORDER BY date DESC, time DESC LIMIT $offset, $total_patvisrecords_per_page";
        $stmt2 = $conn->prepare($sql2);
        if (!$stmt2) {
            die("Error: " . $conn->error);
        }
        $stmt2->bind_param("isssssss", $visit_id, $searcho, $searcho, $docn, $sdate, $ldate, $stime, $ltime);
        if (!$stmt2->execute()) {
    ...
    <div class="container">
        <div class="row">
            <div class="col">
                <h4 class="text-center no-print">Doctor's Observation Chart Records</h4>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <form class="my-3 no-print" action="" method="GET">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="Search by Medicine Modification, Procedure Modification" name="searcho" value="<?php echo $searcho; ?>">
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-sm table-bordered align-middle" style="color:white;">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="text-center align-middle">Doctor's Observation</th>
                            ...
                            <?php while ($row2 = $result2->fetch_assoc()) { ?>
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-center"><?php echo $row2['observe']; ?></td>
