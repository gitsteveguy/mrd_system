...
if(isset($_POST['submit']))
{
    ...
    foreach ($dates as $key => $date) {
        ...
        $therapistins = $therapistnss[$key];
        $therapistid="";
        $therapistna="";
        foreach ($therapistins as $therapistin) {
            $therapistinar = explode(",",$therapistin);
            $therapistid = $therapistid.",".$therapistinar[0];
            $therapistna = $therapistna.",".$therapistinar[1];
        }
        $therapistid = substr($therapistid, 1);
        $therapistna = substr($therapistna, 1);
       ...
       $stmt->bind_param("iisssssssssssiss", $user_id, $visit_id, $date, $nameotreat, $med, $tmtroom, $timein, $timeout, $therapistid, $therapistna, $docremark, $agree, $patsign, $docnid, $docname, $docdet['signature_img']);
        $stmt->execute();
    }
                ...
                <div class="patient">
                    ...
                    <div class="col">
                        ...
                        <label for="therpn">Select Therapist</label>
                                <select name="therpn[0][]" id="therpn" class="form-control stselew select therpn" multiple onchange="maxAllowedMultiselect(this, 3)">
...
?>
    <script>
         function addrow() {
            ...
            patientTemplate = lastPatient;
            var newPatient = patientTemplate.cloneNode(true);
            patientFields.appendChild(newPatient);
            ls = newPatient.querySelector(".therpn");
             nval = Number(ls.name.charAt(7))+1;
            ls.name = setCharAt(ls.name,7,nval);
            ls.id ="therpn"+nval;
            lsid=ls.id;
            $('#'+lsid+" > option").each(function() {
            this.disabled = false;
});
        }
