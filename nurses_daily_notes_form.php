...
if(isset($_POST['submit']))
{
    ...
    foreach ($dates as $key => $date) {
        $time = $times[$key];
        $nursenote = $nursenotes[$key];

        $sql = "INSERT INTO nurse_daily_notes (user_id, visit_id, date, time, nurse_note, nurse_id, nurse_name, nurses_sign) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iissssss", $user_id, $visit_id, $date, $time, $nursenote, $current_user_id, $current_user_name, $current_user_sign);
        $stmt->execute();
    }
 ...
 if (!((!in_array($current_user_role, $table_submit_roles) && !in_array($current_user_role,$sup_roles))||(($formeditable=="No") && (!in_array($current_user_role,$sup_roles)))))
    {
        ?>
<body id="body">
    <h1 class="text-center my-3">Nurse Daily Notes</h1>
        ...
        <form method="POST" action="">
            <div id="patient-fields">
                <div class="patient">
                    ...
                    <div class="col">
                        <div class="form-group">
                            <label for="nursenotes">Nurse Notes</label>
                            <textarea class="textarea" maxlength="200" name="nursenotes[]" class="form-control" value=""></textarea>
                        </div>
                    </div>
            ...
            <button type="button" class="btn btn-primary my-3 mx-2" onclick="addrow()">Add Notes</button>
             <button type="button"class="btn btn-danger" onclick="removerow()">Remove Notes</button>
    ...
    //to be changed for different repeating forms
    $sql = "SELECT COUNT(*) as count FROM $table_name WHERE visit_id=? AND (nurse_note LIKE CONCAT('%', ?, '%')) AND (nurse_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?)";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die("Error: " . $conn->error);
    }
    $stmt->bind_param("issssss", $visit_id, $searcho, $nurn, $sdate, $ldate, $stime, $ltime);
    if (!$stmt->execute()) {
    ...
    $sql2 = "SELECT * FROM $table_name WHERE visit_id=? AND (nurse_note LIKE CONCAT('%', ?, '%')) AND (nurse_name LIKE CONCAT('%', ?, '%')) AND (date BETWEEN ? AND ?) AND (time BETWEEN ? AND ?) ORDER BY date DESC, time DESC LIMIT $offset, $total_patvisrecords_per_page";
        $stmt2 = $conn->prepare($sql2);
        if (!$stmt2) {
            die("Error: " . $conn->error);
        }
        $stmt2->bind_param("issssss", $visit_id, $searcho, $nurn, $sdate, $ldate, $stime, $ltime);
        if (!$stmt2->execute()) {
    ...
    <div class="container">
        <div class="row">
            <div class="col">
                <h4 class="text-center no-print">Nurse Daily Notes</h4>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <form class="my-3 no-print" action="" method="GET">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="Search by Nurse's Notes" name="searcho" value="<?php echo $searcho; ?>">
                            </div>
                        </div>
                    </div>
